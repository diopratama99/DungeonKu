...
PHASE GUIDANCE: ${phaseGuidance(ctx.campaign.phase)}`;
  ...
  return [
    ...
    "",
    `OPTIONS POLICY:`,
    `- Return up to 3 SITUATIONAL options (kind="situational") tailored to this exact scene.`,
    `- The system will prepend template-common options if you return fewer than 3 situational ones.`,
    `- For pivotal moments (shocking reveals, big decisions, important fights): set pivotal_moment=true AND return up to 5 options, all kind="pivotal".`,
    `- IDs must be unique strings within this turn (e.g., "opt_charge", "opt_negotiate").`,
    "",
